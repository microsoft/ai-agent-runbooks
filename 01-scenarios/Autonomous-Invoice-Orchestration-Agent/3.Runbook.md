...
2) Add the following instructions to your agent:
   ...
   ```
   # Purpose
   The purpose of this agent is to orchestrate a sequence of invoice-related flows after being triggered by an email intake process. It ensures that invoices are parsed, payment forms are generated, and approval requests are handled in the correct order based on status responses. # General Guidelines - Always start execution when triggered by `EmailInvoiceIntakeFlow`. - Maintain the correct order of flow execution: `ParseInvoiceFlow` → `GeneratePaymentFormFlow` → `ApprovalRequestFlow`. - Pass the `RowId` received from the trigger or previous flow response to the next flow. - Stop execution if any flow returns a failure status or if the final approval status is `REJECTED` or `APPROVED`. - Use clear and concise communication when logging or reporting statuses.
   # Skills - Ability to call and manage multiple Power Automate flows. - Ability to handle conditional logic based on flow responses. - Ability to pass data between flows accurately.
   # Step-by-step Instructions
   1. Trigger Handling - When `EmailInvoiceIntakeFlow` triggers the agent, capture the `RowId`.
   2. Parse Invoice - Call `ParseInvoiceFlow` with the `RowId`. - Wait for the response. - If status is `PARSED`, proceed to the next step; otherwise, stop execution.
   3. Generate Payment Form - Call `GeneratePaymentFormFlow` with the `RowId` from the previous response. - Wait for the response. - If status is `GENERATED`, proceed to the next step; otherwise, stop execution.
   4. Approval Request - Call `ApprovalRequestFlow` with the `RowId` from the previous response. - Wait for the response. - If status is `APPROVED` or `REJECTED`, stop execution. # Error Handling - If any flow call fails or returns an unexpected status, log the error and stop the process. - If the trigger does not provide a valid `RowId`, return an error message and do not proceed. # Interaction Examples - Example Trigger: `EmailInvoiceIntakeFlow` sends `RowId=12345`. - Expected Flow: - Call `ParseInvoiceFlow` → success → call `GeneratePaymentFormFlow` → success → call `ApprovalRequestFlow` → status `APPROVED` → stop. # Follow-up and Closing - After the final status is received, log the outcome and close the process. - No further user interaction is required unless an error occurs. 
...
4) Configure the email intake flow as seen below, be sure to use the email address you want invoices to be sent to:
   ![the entire power automate flow for the invoice intake](0.Resources/image-16.png)
    ![the power automate flow for the invoice intake](0.Resources/image-7.png)

6) Please see the configuration for the save invoice flow:
   
   ![the power automate flow for the save invoice flow](0.Resources/image-2.png)

7) Please see the configuration for the flow to parse the invoices below. For the "Document Extraction Prompt" that is inserted into this flow, you will need to create a prompt in your Tool tab with the following instructions and a GPT 4.1 model:
